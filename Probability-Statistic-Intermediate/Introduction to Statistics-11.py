...
degrees_zero = []
for f in fahrenheit_degrees:
    degrees_zero.append((f +459.67))
...
smoking_count = 0
count_of_ppl = len(survey_responses)

for val in survey_responses:
    if val in dic:
        smoking_count += dic[val]

average_smoking = smoking_count / count_of_ppl
...
male_list = [savings[val] for val in range(0,len(gender)) if gender[val]=="male"]

female_list = [savings[val] for val in range(0,len(gender)) if gender[val]=="female"]
...
plt.show()
#Average Speed plot with two bins
...
plt.show()
plt.plot(mean_negative)
plt.show()
plt.plot(mean_positive)
plt.show()
...
plt.hist(test_scores_positive)
median_pos = numpy.median(test_scores_positive)
plt.axvline(median_pos,color = 'b')
mean_pos = test_scores_positive.mean()
plt.axvline(mean_pos,color = 'r')
plt.show()
